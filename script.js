...
const openPrototype = (el) => {
  const href = el.dataset.href || el.getAttribute('href') || el.closest('.prototype')?.dataset.href;
...
document.querySelectorAll('.arrow').forEach((b) =>
  b.addEventListener('click', (event) => {
    const name = b.getAttribute('aria-label').replace('Open ', '');
    showToast(name);
    openPrototype(b);
    event.stopPropagation();
  }));
document.querySelectorAll('.prototype').forEach((card) => {
  card.addEventListener('click', (event) => {
    if (event.target.closest('button, a')) return;
    const href = card.dataset.href;
    if (href) {
      showToast(card.querySelector('h3')?.textContent?.trim() || 'Prototype');
      window.location.href = href;
    }
  });
});
